...
//		Point focusPt = new Point(-37.78586543839447,93.64918288360064);//predict fail in k=12 threshold = 20,succ in 99.999
		Point focusPt = new Point(26.11409428117358,21.227538283036267);
...
//		Point focusPt = new Point(-33.27737815442204,85.04308648246376);
		...
		for(int i=0; i<sortedPtn.size(); i++){
			...
			if(ptNum>=k){
				if(guessProb<SimpleMain.THRESHOLD){//go to 100% directly,mostly it won't happen
					guessProb = 100;
				...
				if(guessProb<SimpleMain.THRESHOLD){
...
//					+dist(p3.x, p3.y,focusPt.x , focusPt.y)+")");
	...
	private static boolean getAffectedLists(
			...
			World wd, Point focusPt, Partition boundaryP) throws Exception 
	{
		...
		for(int i=0;i<World.N;i++){
			for(int j=0;j<World.N;j++){
				...
				if (p.minDist < radius && p.maxDist > radius){//yellow list(intesect list)
					...
					if(p.intsecArea<=0){
//						throw new Exception("err intesection area"+p);
						System.err.println("zero intsec Area for "+p);
					}else{
						System.out.println("area is "+ p.intsecArea);
					}
					yellowList.add(p);
