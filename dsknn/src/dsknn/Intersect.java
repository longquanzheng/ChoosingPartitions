...
public class Intersect {
	...
	public static double calIntersect(Partition p, Point focusPt, double radius) throws Exception {
		...
		println("exptOut:"+exptOut + " radius:"+radius);
...
//					throw new Exception("why not intsecNum2 :"+intsecNum+"--:"+intsecPts+" pn:"+p);	
					errprintln("cannot calculate it, return 0");
					return 0;
				}
			case 3: 
				if(intsecNum != 2){
	...
	/**
	 * if size() > i then remove the ext points from them
	 * @param intsecPts
	 * @param p
	 * @param reqNum
	 * @throws Exception 
	 */
	private static void checkExtPts(
			ArrayList<Point> intsecPts, Partition p,int reqNum) throws Exception 
	{
		Point p1,p2,p3,p4;
		...
		int rmNum =  intsecPts.size() - reqNum;
		for(int i=0; i<rmNum; i++){
			boolean found = false;
			for(int j=0; j< intsecPts.size(); j++){
				if(intsecPts.get(j).equals(p1)||intsecPts.get(j).equals(p2)||intsecPts.get(j).equals(p3)||intsecPts.get(j).equals(p4) ){
					intsecPts.remove(j);
					found = true;
					break;
				}
			...
			if(!found){
//				throw new Exception("no pts for remove!!"+i+"-"+intsecPts+"*"+p);
				//TODO I know this is not good but I can only do this now
				//remove the first one
				errprintln("no pts for remove!!"+i+"-"+intsecPts+"*"+p);
				intsecPts.remove(0);
			}
		}
	}
...
//			throw new Exception("why?");
			//TODO cannot calculate it, return 0
			errprintln("cannot calculate it, return 0");
			return 0;
		}
